...
void PowerShellWrapper::ShowBanner() {
    ...
    std::cout << ANSI_WHITE << "     ╚═╝     ╚══════╝" << ANSI_MAGENTA << "          " << ANSI_CYAN << " ╚══╝╚══╝ ╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝" << "\n";
    std::cout << ANSI_CYAN << "                                                                         \n";
    std::cout << ANSI_CYAN << "              " << ANSI_WHITE << "When Powershell becomes safer " << ANSI_CYAN << "│ " << ANSI_WHITE << "Obfuscate & Monitor" << ANSI_CYAN << "              \n";
    std::cout << ANSI_MAGENTA << "    ═══════════════════════════════════════════════════════════════════════\n";
    std::cout << ANSI_RESET << "\n"; 
    std::cout << ANSI_MAGENTA << "    💢" << ANSI_BLUE << "───────────────────────────────────────────────────────────────────" << ANSI_MAGENTA << "💢\n";
    std::cout << ANSI_BLUE << "        " << ANSI_WHITE << "Version:    " << ANSI_GREEN << "v1.0.0" 
    ...
    std::cout << ANSI_BLUE << "        " << ANSI_WHITE << "Status:  " << ANSI_YELLOW << "⚡ Ready" 
            << ANSI_BLUE << "   │  " << ANSI_WHITE << "Mode: " << ANSI_CYAN << "Enigma" 
            << ANSI_BLUE << std::string(65, ' ') << "\n";
    std::cout << ANSI_MAGENTA << "    💢" << ANSI_BLUE << "───────────────────────────────────────────────────────────────────" << ANSI_MAGENTA << "💢\n";
    std::cout << ANSI_RESET << "\n";
...
void PowerShellWrapper::ShowHelp() {
    ...
    std::cout << "      exit            - Terminate session wrapper\n\n";
...
std::string PowerShellWrapper::ReadInputWithHistory() {
    ...
    while (true) {
        ...
        if (ch == -32 || ch == 0 || (unsigned char)ch == 224) {
            ...
            if (ch == -32 || (unsigned char)ch == 224) {
                ...
                if (nextCh == 72) { // UP
                    ...
                    // Save old buffer length and get new command
                    size_t oldLength = buffer.length();
                    buffer = m_commandHistory[historyIndex];
                    
                    // Clear old line by backspacing through each character
                    for (size_t i = 0; i < oldLength; i++) {
                        std::cout << "\b \b";  // Backspace, space, backspace
                    }
                    std::cout.flush();
                    ...
                    // Save old buffer length
                    size_t oldLength = buffer.length();
                    
                    // Fetch new buffer content first
        ...
        if (!m_authManager.IsAuthenticated() && m_obfuscator.IsEnabled()) {
            showClear = false;
            std::vector<std::string> whitelist = {"setkey", "openme", "help", "exit", "status", "enable", "timer", "history", "clearlog", "exportlog"};
    ...
    if (!m_authManager.IsAuthenticated() && m_obfuscator.IsEnabled()) {
        ...
        std::vector<std::string> whitelist = {"setkey", "openme", "help", "exit", "status", "enable"};
